...
const productFilterByCategory = ( category : CategoryType)=>{
  if(category === CategoryType.All){
    return products.filter((product)=> product.status === "active").length
  }
  return products.filter(
     product => product.status === "active" && product.category === category).length
}
// Categorías de productos
export const categories: Category[] = [
  { id:  CategoryType.All, name: 'Todos', icon: 'apps', count: productFilterByCategory(CategoryType.All) },
  { id: CategoryType.Tech, name: 'Tecnología', icon: 'devices', count: productFilterByCategory(CategoryType.Tech) },
